...
func (r *repositoryImpl) CreateInvitation(ctx context.Context, inv *Invitation) error {
	query := `
		...
		VALUES (?, ?, ?, ?, ?, NOW(), NOW())
	`
	res, err := r.db.ExecContext(ctx, query, inv.OrgID, inv.RoleID, inv.Email, inv.Token, inv.ExpiresAt)
	if err != nil {
		return fmt.Errorf("failed to insert invitation: %w", err)
	}
	inv.ID, err = res.LastInsertId()
	if err != nil {
		return fmt.Errorf("failed to get inserted invitation ID: %w", err)
	}
	return nil
}
...
func (r *repositoryImpl) GetInvitationByToken(ctx context.Context, token string) (*Invitation, error) {
	var inv Invitation
	query := `SELECT id, org_id, role_id, email, token, expires_at, created_at FROM invitations WHERE token = ?`
...
func (r *repositoryImpl) UpdateMemberRole(ctx context.Context, orgID, userID, roleID int64) error {
	query := `UPDATE org_members SET role_id = ?, updated_at = NOW() WHERE org_id = ? AND user_id = ?`
...
func (r *repositoryImpl) RemoveMember(ctx context.Context, orgID, userID int64) error {
	query := `DELETE FROM org_members WHERE org_id = ? AND user_id = ?`
...
func (r *repositoryImpl) CreateUser(ctx context.Context, user *User) (int64, error) {
	query := `
		...
		VALUES (?, ?, ?, ?)
	`
	res, err := r.db.ExecContext(ctx, query, user.CognitoSub, user.Email, user.FirstName, user.LastName)
	if err != nil {
...
func (r *repositoryImpl) GetUserByEmail(ctx context.Context, email string) (*User, error) {
	var user User
	query := `SELECT id, cognito_sub, email, first_name, last_name, created_at, updated_at FROM users WHERE email = ?`
...
// AddUserToOrg inserts a new record into the 'org_members' table.
func (r *repositoryImpl) AddUserToOrg(ctx context.Context, member *OrgMember) error {
	query := `
		...
		VALUES (?, ?, ?, ?)
	`
	res, err := r.db.ExecContext(ctx, query, member.OrgID, member.UserID, member.RoleID, member.Status)
	if err != nil {
		return fmt.Errorf("failed to add user to organization: %w", err)
	}
	member.ID, err = res.LastInsertId()
	if err != nil {
