...
export default function RFQShipmentHandoff({
  ...
  if (shipmentHandoffData === null) {
    return (
      <div className="bg-white border border-slate-200 rounded-xl p-12 text-center shadow-xs" data-testid="rfq-shipment-loading">
        <div className="w-12 h-12 bg-teal-50 border border-teal-100 rounded-2xl flex items-center justify-center mx-auto mb-3.5 text-teal-600">
          <Loader2 className="w-6 h-6 animate-spin text-teal-600" />
        </div>
        <h3 className="text-sm font-bold text-slate-900 mb-1">Loading Shipment Execution Handoff...</h3>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-teal-50 text-teal-700 border border-teal-200/60">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 pt-1 border-t border-slate-100">
          ...
          >
            <div className="min-w-0">
              <span className="text-[10px] font-extrabold text-teal-600 uppercase block">5. Live Shipment</span>
      ...
      {!activeShipment && (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className={`p-4 rounded-xl border ${isQuoteApproved ? 'bg-emerald-50/50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Step 1 • Commercial</span>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${isQuoteApproved ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
            ...
            <div className={`p-4 rounded-xl border ${isBookingConfirmed ? 'bg-emerald-50/50 border-emerald-200' : isBookingCreated ? 'bg-blue-50/50 border-blue-200' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Step 2 • Booking</span>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${isBookingConfirmed ? 'bg-emerald-100 text-emerald-800' : isBookingCreated ? 'bg-blue-100 text-blue-800' : 'bg-slate-200 text-slate-700'}`}>
                  {sourceBooking ? sourceBooking.status : 'NOT INITIATED'}
                </span>
              </div>
              <h4 className="text-xs font-bold text-slate-900 mb-1 font-mono">
            ...
            <div className={`p-4 rounded-xl border ${isBookingConfirmed ? 'bg-teal-50/70 border-teal-200' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  {activeShipment.closure_status && activeShipment.closure_status !== 'ACTIVE' && (
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${
                      activeShipment.closure_status === 'CLOSED' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-indigo-50 text-indigo-700 border-indigo-200'
                    }`}>
                      {activeShipment.closure_status === 'CLOSED' ? 'CLOSED' : 'READY FOR CLOSURE'}
                    </span>
                  )}
                  {activeShipment.active_exceptions_count > 0 && (
                    <span className="px-2 py-0.5 rounded bg-red-100 text-red-700 text-xs font-bold border border-red-200 flex items-center gap-1" title={`${activeShipment.active_exceptions_count} active exception(s)`}>
                      <AlertTriangle className="w-3.5 h-3.5 text-red-600" /> {activeShipment.active_exceptions_count} Active Exceptions
                    </span>
                  )}
                </div>
                <p className="text-xs text-slate-500 flex items-center gap-2 mt-0.5">
                  <span>Carrier: <strong className="text-slate-700">{activeShipment.carrier_name || activeShipment.carrier_scac}</strong></span>
                  ...
                  {activeShipment.booking_number && (
                    <span>• Source Booking: <strong className="font-mono text-slate-700">{activeShipment.booking_number}</strong></span>
                  )}
                </p>
              </div>
            </div>
              ...
              {[
                ...
                return (
                  <div key={step.id} className="flex flex-col items-center text-center relative z-10">
                    ...
                    <span className={`text-xs font-bold ${state === 'active' ? 'text-teal-600' : state === 'completed' ? 'text-emerald-700' : 'text-slate-500'}`}>
                      {step.label}
                    </span>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/70">
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-800">{activeShipment.origin_port}</p>
                </div>
                <div className="border-t border-slate-200 pt-2">
                  <span className="text-[10px] font-semibold text-slate-400 uppercase">Destination Port</span>
                  <p className="text-sm font-bold text-slate-800">{activeShipment.destination_port}</p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {activeShipment.container_numbers && activeShipment.container_numbers.length > 0 ? (
                      activeShipment.container_numbers.map((c, i) => (
                        <span key={i} className="font-mono text-xs px-2 py-0.5 rounded bg-slate-200 text-slate-800 font-bold">
                          {c}
                        </span>
                      ))
                    ) : (
                ...
                <div className="border-t border-slate-200 pt-2 flex items-center justify-between text-xs">
                  <div>
                    ...
                    <span className="font-mono text-slate-700">{activeShipment.mbl_number || 'Pending'}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] font-semibold text-slate-400 uppercase block">HBL Number</span>
                    <span className="font-mono text-slate-700">{activeShipment.hbl_number || 'Pending'}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="divide-y divide-slate-100">
            {shipments.map(s => (
              <div key={s.id} className="p-3.5 flex flex-wrap items-center justify-between gap-3 text-xs">
                <div className="flex items-center gap-3">
                  <span className="font-bold text-slate-900">Shipment #{s.id}</span>
                  <span className="text-slate-600 font-medium">{s.carrier_name || s.carrier_scac}</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {s.active_exceptions_count > 0 && (
                    <span className="px-1.5 py-0.5 rounded bg-red-100 text-red-700 text-[10px] font-bold border border-red-200 flex items-center gap-1" title={`${s.active_exceptions_count} active exception(s)`}>
